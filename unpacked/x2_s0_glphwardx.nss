...
void DoDamage(int nDamage, object oTarget, int nCL)
{
    effect eVis  = EffectVisualEffect(VFX_IMP_SONIC);
    effect eDam  = EffectDamage(nDamage, DAMAGE_TYPE_SONIC);
    effect eHide = EffectSkillDecrease(SKILL_HIDE, 25);
    effect eMS   = EffectSkillDecrease(SKILL_MOVE_SILENTLY, 25);
    ...
    if(nDamage > 0)
    {
        ...
        DelayCommand(0.02, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectLinkEffects(eHide, eMS), oTarget, RoundsToSeconds(nCL)));
    }
}
...
void main()
{
  //Declare major variables
    ...
    {
        SetLocalInt(GetArea(OBJECT_SELF), "GLYPH_NUM", GetLocalInt(GetArea(OBJECT_SELF), "GLYPH_NUM") - 1);
    ...
    while(GetIsObjectValid(oTarget))
    {
            if (spellsIsTarget(oTarget,SPELL_TARGET_STANDARDHOSTILE,oCreator))
            {
                ...
                if (!MyResistSpell(oCreator, oTarget))
                {
                    ...
                    {
                        AssignCommand(oCreator, DoDamage(nDamage,oTarget, nCasterLevel));
                    }
                    else
                    {
                        DoDamage(nDamage,oTarget, nCasterLevel);
                    }
