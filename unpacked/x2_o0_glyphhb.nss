...
void main()
{
    ...
    if (bSetup == 0)
    {
        ...
        if (nMetaMagic == METAMAGIC_EXTEND)
        {
           nDuration =           nDuration *2;//Duration is +100%
            ...
            // show glyph symbol only for 3 seconds
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY,EffectVisualEffect(445),OBJECT_SELF,3.0f);
    ...
    if(GetLocalInt(GetArea(OBJECT_SELF), "GLYPH_NUM") > 3)
    {
        SendMessageToPC(oCaster, "An area can have a maximum of three Glyphs of Warding.");
